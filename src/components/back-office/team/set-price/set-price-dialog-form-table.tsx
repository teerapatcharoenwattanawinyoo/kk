...
export default function SetPriceDialogFormTable({
  ...
  const handleApplyPrice = () => {
    ...
    createPriceSetMutation.mutate(requestData, {
      ...
      onError: (error) => {
        ...
        setIsApplying(false);
        // Handle error case - you might want to show an error message to user
      },
    });
  };
  ...
  return (
    <>
      ...
      >
        <DialogContent className="max-w-3xl gap-4 rounded-lg bg-card p-0">
          {/* Header with Title and Close Button */}
          <div className="flex items-center justify-between px-6 py-4">
            <DialogHeader>
              <DialogTitle className="mt-4 text-xl font-semibold text-[#355FF5]">
                Select price group
              </DialogTitle>
            </DialogHeader>
          </div>
          ...
          <div className="px-6 py-6">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h2 className="relative px-4 text-xl font-semibold text-[#364A63]">
                  General Price
                </h2>
              </div>
