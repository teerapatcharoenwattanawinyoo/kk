...
import { useCreateTeam, useTeamHostId } from '../_hooks/use-teams'
import { TeamFormData } from '../_schemas/team.schema'
import TeamAddForm from './_components/team-add-form'
      ...
      <div className="shadow-xs mx-auto flex max-w-4xl flex-col rounded-lg bg-card p-4 md:p-8">
        ...
        <div className="flex items-center justify-between border-b pb-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-title text-xl font-medium md:text-2xl">{t('team.add_team')}</h1>
          </div>
          ...
          >
            {createTeamMutation.isPending ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-light uppercase">{t('team.submitting')}</span>
              </div>
            ) : (
              <p className="text-sm font-normal uppercase">{t('buttons.submit')}</p>
            )}
          </Button>
        </div>
