...
export const ReceiptPreviewDialog = ({
  ...
  // Generate receipt ID from invoice prefix data
  const generateReceiptId = () => {
    const invoiceData = taxInvoiceReceiptData?.data

    // ถ้าไม่มีข้อมูลจาก API ให้ใช้ default
    if (!invoiceData) {
      return 'RCT2837183919313'
    }

    // ใช้ค่าจริงจาก API
    const header = invoiceData.header_document || ''
    const center = invoiceData.center_document || ''
    const end = invoiceData.end_document || ''

    // ถ้าทุกค่าเป็นค่าว่างให้ใช้ default
    if (!header && !center && !end) {
      return 'RCT2837183919313'
    }

    // Generate based on format_document type
    switch (invoiceData.format_document) {
      case 1: // Starter INV
      ...
      case 2: // YY/MM/DD
        ...
        return `${header}${yy}/${mm}/${dd}${center}${end}`
      case 3: // YY/MM
        const nowYM = new Date()
        const yyYM = nowYM.getFullYear().toString().slice(-2)
        const mmYM = (nowYM.getMonth() + 1).toString().padStart(2, '0')
        return `${header}${yyYM}/${mmYM}${center}${end}`
      case 4: // Year
        const year = new Date().getFullYear()
        return `${header}${year}${center}${end}`
      default:
        return `${header}${center}${end}`
    }
  }
