    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon">
          <Sun className="text-oc-sidebar h-[1.2rem] w-[1.2rem] rotate-0 scale-100 transition-all dark:-rotate-90 dark:scale-0" />
          <Moon className="text-oc-sidebar absolute h-[1.2rem] w-[1.2rem] rotate-90 scale-0 transition-all dark:rotate-0 dark:scale-100" />
          <span className="sr-only">Toggle theme</span>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-44">
        <DropdownMenuItem onClick={() => onPickTheme('light')} disabled={theme === 'light'}>
          <Sun className="h-4 w-4 text-muted-foreground" />
          <span>Light</span>
          {theme === 'light' && <Check className="ml-auto h-4 w-4 text-foreground" />}
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => onPickTheme('dark')} disabled={theme === 'dark'}>
          <Moon className="h-4 w-4 text-muted-foreground" />
          <span>Dark</span>
          {theme === 'dark' && <Check className="ml-auto h-4 w-4 text-foreground" />}
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => onPickTheme('system')} disabled={theme === 'system'}>
          <Laptop className="h-4 w-4 text-muted-foreground" />
          <span>System</span>
          {theme === 'system' && <Check className="ml-auto h-4 w-4 text-foreground" />}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
...
export const ThemeToggleButton = ({
  ...
  return (
    <Button
      ...
      onClick={handleClick}
      className={cn(
        'text-co-sidebarrelative overflow-hidden transition-all',
        showLabel && 'gap-2',
        className,
      )}
