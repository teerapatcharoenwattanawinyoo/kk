...
import type { LoginResponse } from '@/app/[locale]/(auth)/_services'
import { COOKIE_KEYS } from '@/lib/constants'
import { serverApi } from '@/lib/server/backend'
import { formatPhoneForAPI } from '@/lib/utils/phone'
import { cookies } from 'next/headers'
import { redirect } from 'next/navigation'
import { BackendLoginSchema, SignInSchema } from '../_schemas/sign-in.schema'
...
const signInWithCredentials = async (input: {
  email?: string
  phone?: string
  password: string
  ...
  const secure = false
  // const secure = process.env.NODE_ENV === 'production' // Set to true in production HTTPS
  console.log('🚀 ~ signInWithCredentials ~ secure:', secure)
...
export { signInAction, signInWithCredentials }
