...
import { getPriceSet, PriceGroup, UpdatePriceRequest } from '@/lib/api/team-group/price-groups'
...
export default function EditPriceGroup() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!teamHostId || !priceId) return
      try {
        const res = await getPriceSet('general', 1, 100)
        const found = res.data.data.find((g: PriceGroup) => g.id.toString() === priceId)
        if (found) {
          setInitialData({
            form: {
            ...
            feeForm: {
              ...
              applyAfterMinute: found.charging_fee?.apply_after_minute?.toString() || '',
              minuteFeeDescription: found.minute_fee?.description || '',
              feePerMin: found.minute_fee?.fee_per_min || '',
              applyFeeAfterMinute: found.minute_fee?.apply_fee_after_minute?.toString() || '',
              feeStopsAfterMinute: found.minute_fee?.fee_stops_after_minute?.toString() || '',
              ...
              maxTotalIdleFee: found.idle_fee?.max_total_idle_fee?.toString() || '',
            },
  ...
  const handleSubmit = async (data: {
    ...
    if (!teamHostId || !priceId) {
      toast({
        title: 'Error',
        description: 'Team host ID or priceId not found. Please try again.',
        variant: 'destructive',
      })
      return
    }
