...
export async function getStationCategories(): Promise<StationCategoriesResponse> {
  try {
    const response = await api.get<StationCategoriesResponse>(
      API_ENDPOINTS.STATION.CATEGORIES,
    );
    return response;
  } catch (error) {
...
export async function createChargingStation(
  ...
  try {
    ...
    data.work.forEach((workItem, index) => {
      ...
      formData.append(
        `work[${index}][work_time_start]`,
        workItem.work_time_start,
      );
    ...
    if (data.images && data.images.length > 0) {
      console.log("🔥 Processing images array:", data.images);
    ...
    const response = await api.post<CreateChargingStationResponse>(
      url,
      formData,
      {
        headers: {
...
export async function updateChargingStation(
  ...
  try {
    ...
    // Add contact field (always send, even if empty)
    console.log(
      "🔥 Contact value from data (UPDATE):",
      data.contact,
      "Type:",
      typeof data.contact,
    );
    ...
    // Add images as File objects (use standard field naming)
    console.log("🔥 data.images (UPDATE):", data.images);
    console.log("🔥 data.images length (UPDATE):", data.images?.length);
    if (data.images && data.images.length > 0) {
      console.log("🔥 Processing images array (UPDATE):", data.images);
      data.images.forEach((image, index) => {
    ...
    const response = await api.patch<UpdateChargingStationResponse>(
      url,
      formData,
      {
        headers: {
...
export async function getChargingStationDetail(
  ...
  try {
    ...
    console.log(
      "🔥 Partner station work:",
      response.data?.partner_station_work,
    );
    console.log(
      "🔥 Partner station description:",
      response.data?.partner_station_description,
    );
    console.log(
      "🔥 Partner station gallery:",
      response.data?.partner_station_gallery,
    );
...
export function convertStationDetailToFormData(
  ...
  if (detail.partner_station_work && detail.partner_station_work.length > 0) {
    console.log(
      "🔥 Converting partner_station_work:",
      detail.partner_station_work,
    );
    ...
    console.warn(
      "⚠️ No partner_station_work data found, using default openClose",
    );
  }
