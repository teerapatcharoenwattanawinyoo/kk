...
export const getChargerBrands = async (): Promise<ChargerBrandsResponse> => {
  try {
    const response = await api.get<ChargerBrandsResponse>(
      API_ENDPOINTS.CHARGER.BRANDS,
    );
    return response;
  } catch (error) {
...
export const updateSerialNumber = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to update serial number: ${errorData.message || "Unknown error"}`,
    );
  }
...
export const deleteCharger = async (
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      `Failed to delete charger: ${errorData.message || "Unknown error"}`,
    );
  }
