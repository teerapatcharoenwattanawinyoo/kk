...
} from "@/lib/api/team-group/charging-station";
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [
          ...QUERY_KEYS.STATION,
          "list",
          { team_group_id: variables.team_group_id },
        ],
      });
      ...
      if (data.data?.partner_station_gallery?.length > 0) {
        console.log(
          "Station images uploaded:",
          data.data.partner_station_gallery,
        );
      }
    },
    onError: (error) => {
...
export function useDeleteChargingStation() {
  ...
  return useMutation({
    mutationFn: (stationId: string | number) =>
...
export function useDeleteStationImage() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, partnerStationGalleryId) => {
      console.log(
        `Station image ${partnerStationGalleryId} deleted successfully`,
      );
...
export function useChargingStationFormData(stationId: string | number | null) {
  const { data: stationDetail, ...queryResult } =
    useChargingStationDetail(stationId);

  const formData = stationDetail
    ? convertStationDetailToFormData(stationDetail)
    : null;
